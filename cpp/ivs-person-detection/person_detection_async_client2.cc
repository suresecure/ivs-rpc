...
public:
  ...
  void ObjectDetection(const ImageRegion &image_region) {
  ...
  void AsyncCompleteRpc() {
    ...
    while (cq_.Next(&got_tag, &ok)) {
      ...
      if (call->status.ok())
        std::cout << "Greeter received: " << call->general_reply.error_code() << std::endl;
      else
...
private:
  ...
  struct AsyncClientCall {
    ...
    std::unique_ptr<ClientAsyncResponseReader<ObjectDetectionReply>>
        response_reader;
  };
...
int main(int argc, char **argv) {
  ...
  ImageAnalysisClient greeter(grpc::CreateChannel(
      "localhost:50051", grpc::InsecureChannelCredentials()));

  // Spawn reader thread that loops indefinitely
  std::thread thread_ =
      std::thread(&ImageAnalysisClient::AsyncCompleteRpc, &greeter);
